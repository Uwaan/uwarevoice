...
class VCEngine:
    ...
    def __init__(self) -> None:
        ...
        self.use_fp32 : bool = True
        self.use_jit : bool = False
        self.infer_device = "cuda:0"
        self.model_name : str = "defaultvoice"
        self.model_filename : str = ""

        #RVCv2 specific parameters
        self.rvc_idx_filename : str = ""
        self.rvc_pth_filename : str = ""
        self.rvc_index_rate : float = 0.0
        self.num_cpus : int = 4
        self.f0_method : str = "rmvpe"
        self.config : Config = None
        self.inp_q : Queue = None
        self.opt_q : Queue = None

        self._settings: dict = {}
        self.input_level_db : int = -144
        self.hot_update = False

        # fairseq/hubert are ancient and PyTorch >=2.6 really seems to dislike them fyi
        torch.serialization.add_safe_globals([fairseq.data.dictionary.Dictionary])

    def configure(self, settings: dict) -> None:
        self._settings = settings

        self.input_gain = settings.get("input_volume", 1.0)
        self.output_gain = settings.get("output_volume", 1.0)
        ...
        self.use_fp32 = settings.get("use_fp32", True)
        ...
        # Gain, gating should be performed by client if using client
        # audio devices.
        if settings.get("input_device", "client") == "client":
            self.input_gain = 1.0
            self.input_gate_db = -120   # Effectively non-functional
    ...
    def start_vc(self):
        if self.vc_active:
            logging.warning("(vc_engine): Conversion already active!")
            return

        torch.cuda.empty_cache()

        # Build the Config object rtrvc.RVC expects. Only the fields actually
        # consulted by rtrvc are populated; everything else is left default.
        self.config = Config(
            device=self.infer_device,
            is_half=not self.use_fp32,
            use_jit=self.use_jit,
            dml=False,
        )

        # inp_q / opt_q are only used by the multi-process Harvest f0
        # path in rtrvc. We don't spawn Harvest workers (we use rmvpe/fcpe/
        # crepe by default), but rtrvc wants to have them anyway.
        if self.inp_q is None:
            self.inp_q = Queue()
            self.opt_q = Queue()

        # Reuse the previous RVC so the hubert/rmvpe models don't reload on
        # every hot-reconfigure.
        prev_rvc = self.vc if isinstance(self.vc, rtrvc.RVC) else None
        ...
        # an audio device but may need to be multiplexed if
        ...
        # rtrvc.RVC takes 16k audio in and emits at its trained tgt_sr.
        self.model_input_sr = 16000
        self.model_output_sr = int(self.vc.tgt_sr)
        model_dtype = "float16" if self.vc.is_half else "float32"

        logging.info(f"(vc_engine) Model is requesting {model_dtype} data " +
        ...
        #    max(self.crossfade_size_samples, 4 * self.hop_length_output)
        #)
        # TODO: scale search range with crossfade window?
        ...
        self.sola_buffer: torch.Tensor = torch.zeros(
            self.crossfade_size_samples, device=self.infer_device, dtype=torch.float32
        )
        ...
        self.fade_in_window: torch.Tensor = (
            torch.sin(
                        ...
                        steps=self.crossfade_size_samples,
                        device=self.infer_device,
                        dtype=torch.float32,
                    )
    ...
    def on_chunk_received(self, in_chunk: np.ndarray) -> Tuple[np.ndarray, float, float]:
        ...
        if not self.vc_active:
            logging.warning("(vc_engine): Audio received for processing, but conversion not running!")
            return np.zeros(in_chunk.shape[-1]), 0.0, 0.0

        # Stream from network should already be mono, but
        ...
        if self.input_gain != 1.0:
            # Multiply all elements by scalar should just work like this?
            in_chunk = in_chunk * self.input_gain
...
#                                       hop_length=self.hop_length)[:, 2:]  # [:, 2:] discards first 2 hop?
        ...
        # TODO: implementing a "gate tail" - about a half second grace
        # period before input completely cuts - worked out very well
        # on the client side; implement the same here?
        ...
        output_frame = self.vc.infer(self.input_frame_resampled,
                                     self.chunk_resampled_samples,
                                     self.skip_head,
                                     self.return_length,
                                     self.f0_method)
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! #
        ...
        # to figure out what's going on. It is a method of reducing
        # crossfade artifacts by mathematically evaluating the point
        # in time where the two waveforms synchronize best.
        #
        ...
        sola_chunk = output_frame[None, None, :self.crossfade_size_samples + self.sola_search_samples]
        # Correlation point will be at offset = Σ(a . b) / sqrt(Σ(a^2) . Σ(b^2))
        a = torch.nn.functional.conv1d(sola_chunk, self.sola_buffer[None, None, :])
        b = torch.sqrt(
            torch.nn.functional.conv1d(
                sola_chunk**2,
                torch.ones(1, 1, self.crossfade_size_samples, device=self.infer_device),
            ) + 1e-8        # Prevent NaN
        )
        offset = torch.argmax(a[0, 0] / b[0, 0])
        ...
        # period
        if self.use_vocoder:
            output_frame[: self.crossfade_size_samples] = phase_vocoder(
                self.sola_buffer,
                output_frame[: self.crossfade_size_samples],
                self.fade_out_window,
                self.fade_in_window,
            )
        else:
            output_frame[: self.crossfade_size_samples] *= self.fade_in_window
            output_frame[: self.crossfade_size_samples] += (
        ...
        self.sola_buffer[:] = output_frame[
            self.chunk_output_samples : self.chunk_output_samples + self.crossfade_size_samples
        ]
    ...
    def set_param(self, key, value):
        if key in self._settings:
            ...
            # Setting these in this class's fields will produce
            # an immediate change because they're just variables/
            # flags used in chunk processing
            if key == "input_gate_db":
                self.input_gate_db = value
            elif key == "input_volume":
                self.input_gain = value
            elif key == "output_volume":
                self.output_gain = value
            elif key == "use_output_rms":
                self.use_output_rms = value
            elif key == "use_noise_reduction":
                self.use_noise_reduction = value
            elif key == "use_vocode_smoothing":
                self.use_vocoder = value
            elif key == "pitch_shift_semitones":
                # Pitch / formant shift do require rebuilding the VC object, but
                # do not require reloading objects/reconfiguring buffers/etc
                # and the rest of the VC object (loaded model, etc) remains
                # set up
                self.input_pitch_shift = value
                if self.vc_active:
                ...
                self.input_formant_shift = value
                if self.vc_active:
                    self.hot_update = True
                    self.vc = rtrvc.RVC(
                        self.input_pitch_shift,
    ...
    def hot_reload(self, key, value):
        if key in self._settings:
            ...
            if self.vc_active:
                ...
                logging.warning(
                    "(vc_engine): Non-trivial settings changed with engine active. "
                    + "Hot reload of engine may have unintended side effects.")
                self.stop_vc()

            self._settings[key] = value
            self.configure(self._settings)  # Rebuilds buffers, VC object, etc
